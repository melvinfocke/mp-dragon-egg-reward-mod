...
import com.melvinfocke.idiotenspielplatz.inventories.BackpackSlot;
...
public class BackpackScreenHandler extends ScreenHandler {
    public static final Identifier IDENTIFIER = new Identifier(Idiotenspielplatz.MOD_ID, "backpack");
    private final BackpackInventory inv;

    public BackpackScreenHandler(PlayerInventory playerInv, int syncId, BackpackInventory inv) {
        super(Idiotenspielplatz.BACKPACK_SCREEN_HANDLER, syncId);
        this.inv = inv;
    ...
    public void onSlotClick(int i, int j, SlotActionType type, PlayerEntity player) {
        if (type != SlotActionType.CLONE) {
            if (i >= 0 && player.getInventory().selectedSlot + 27 + this.inv.size() == i) return;
        }
